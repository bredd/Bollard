...
// TODO: Convert this to the new COM interop pattern
...
#if !RAW_PROPERTY_STORE
            m_contentType = GetValue(s_pkContentType) as string ?? string.Empty;
#endif
        }
    ...
    public class PropertyDescription
    {
        ...
        internal PropertyDescription(PropertyStoreInterop.IPropertyDescription iPropertyDescription)
        {
            ...
            try
            {
                ...
                {
                    m_canonicalName = Marshal.PtrToStringUni(pszName) ?? m_propertyKey.ToString();
                }
                else
                {
                    Marshal.ThrowExceptionForHR(hResult);
                    m_canonicalName = string.Empty;
                }
            }
            finally
            {
            ...
            try
            {
                ...
                if (hResult >= 0 && pszName != IntPtr.Zero)
                {
                    m_displayName = Marshal.PtrToStringUni(pszName) ?? m_propertyKey.ToString();
                }
                else
                {
                    m_displayName = m_propertyKey.ToString();
                }
            }
            finally
            {
